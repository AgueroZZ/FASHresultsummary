...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(sigmaSmooth); // theta = -2log(sigma)
  // Type thetaIWP = -2*log(sigmaSmooth);
  ...
  // Transformations
  vector<Type> eta = log_offset + X * beta + B * U;
  ...
  if(sigmaSmooth != 0){
    vector<Type> PU = P*U;
    Type UPU = (U * PU).sum();
    lpW += -0.5 * exp(-2*log(sigmaSmooth)) * UPU; // U part
  }
  ...
  // Log determinant
  if(sigmaSmooth != 0){
    Type logdet1 = d * (-2*log(sigmaSmooth)) + logPdet;
