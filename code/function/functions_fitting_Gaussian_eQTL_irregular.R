...
compute_log_likelihood_ospline <- function(x, y, p, num_knots = 100, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian = 0.1){
  knots <- seq(1, 16, length=num_knots)
  X <- BayesGP:::global_poly_helper(x, p = p)
...
fit_ospline <- function(x, y, p, num_knots = 100, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian = 0.1, refined_x = NULL){
  knots <- seq(1, 16, length=num_knots)
  X <- BayesGP:::global_poly_helper(x, p = p)
  P <- BayesGP::compute_weights_precision_helper(knots)
  B <- BayesGP:::local_poly_helper(knots = knots, refined_x = x, p = p)
  ...
  samps_coef <- LaplacesDemon::rmvnp(n = 8000, mu = mod$mean, Omega = as.matrix(mod$prec))
  if(!is.null(refined_x)){
    B <- BayesGP:::local_poly_helper(knots = knots, refined_x = refined_x, p = p)
    X <- BayesGP:::global_poly_helper(refined_x, p = p)
  }
  samps_fitted <-as.matrix(B) %*% t(samps_coef[,1:ncol(B)]) + as.matrix(X) %*% t(samps_coef[,(ncol(B)+1):ncol(samps_coef)])
  }
  else{
...
fit_ospline_with_prior <- function(x, y, p, num_knots, prior_weight, pred_step, betaprec = 0.001, sd_gaussian = 0.1, num_cores = 1, refined_x = NULL) {
  ...
  if (num_cores > 1 && .Platform$OS.type != "windows") {
    ...
    results_list <- mclapply(seq_len(nrow(active_prior)), function(i) {
      ...
      fit_ospline(x = x, y = y, p = p, num_knots = num_knots, psd_iwp = psd_iwp, pred_step = pred_step, betaprec = betaprec, sd_gaussian = sd_gaussian, refined_x = refined_x)
    }, mc.cores = num_cores)
  }
  else {
    ...
    for (i in seq_len(nrow(active_prior))) {
      psd_iwp <- active_prior$psd_iwp[i]
      results_list[[i]] <- fit_ospline(x = x, y = y, p = p, num_knots = num_knots, psd_iwp = psd_iwp, pred_step = pred_step, betaprec = betaprec, sd_gaussian = sd_gaussian, refined_x = refined_x)
    }
  }
...
fit_ospline_with_prior2 <- function(x, y, num_knots, prior_weight, pred_step, betaprec = 0.001, sd_gaussian = 0.1, num_cores = 1, refined_x = NULL) {
  # Filter out rows where prior weight is zero
  active_prior <- prior_weight[prior_weight$prior_weight > 0, ]
  ...
  if (num_cores > 1 && .Platform$OS.type != "windows") {
    ...
    results_list <- mclapply(seq_len(nrow(active_prior)), function(i) {
      ...
      fit_result <- fit_ospline(x = x, y = y, p = row$p, num_knots = num_knots, psd_iwp = row$psd_iwp, pred_step = pred_step, betaprec = betaprec, sd_gaussian = sd_gaussian, refined_x = refined_x)
      fit_result
    }, mc.cores = num_cores)
    # Extract log likelihoods from results
