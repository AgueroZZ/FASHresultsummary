...
simulate_nonlinear_function <- function(n_basis = 5, sd_function = 1) {
...
simulate_linear_function <- function(sd_function = 1){
  beta0 <- rnorm(1, mean = 0, sd = sd_function)
  beta1 <- rnorm(1, mean = 0, sd = sd_function)
  function(x_new) {
...
simulate_nondynamic_function <- function(sd_function = 1){
  beta0 <- rnorm(1, mean = 0, sd = sd_function)
  function(x_new) {
    return(beta0)
  }
...
simulate_process <- function(n_basis = 5, sd_fun = 1, sd = 0.1, type = "nonlinear"){
  if(type == "linear"){
    g <- simulate_linear_function(sd_function = sd_fun)

  }
  else if(type == "nonlinear") {
    g <- simulate_nonlinear_function(n_basis = n_basis, sd_function = sd_fun)
  }
  else if(type == "nondynamic") {
    g <- simulate_nondynamic_function(sd_function = sd_fun)
  }
  else {
