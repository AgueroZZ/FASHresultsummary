...
compute_log_likelihood_exact <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001){
...
fit_exact <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001){
  C <- as.matrix(compute_Wp_cov(x, p = p))
  P = as(solve(C),'dgTMatrix')
  n <- length(x)
  X <- BayesGP:::global_poly_helper(x, p = p)
...
compute_log_likelihood_exact_aug <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001){
...
fit_exact_aug <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001){
  P = Compute_Aug_Wp_Prec(x, p = p)
  X = BayesGP:::global_poly_helper(x, p = p)
  B = Compute_design_Aug(x, p = p)
  tmbdat <- list(
...
compute_log_likelihood_ospline <- function(x, y, p, num_knots = 100, psd_iwp, pred_step, betaprec = 0.001){
...
fit_ospline <- function(x, y, p, num_knots = 100, psd_iwp, pred_step, betaprec = 0.001){
  knots <- seq(min(x), max(x), length=num_knots)
  X <- BayesGP:::global_poly_helper(x, p = p)
  P <- BayesGP::compute_weights_precision_helper(knots)
  B <- BayesGP:::local_poly_helper(knots = knots, refined_x = x, p = p)
...
compute_log_likelihood_ospline_seq <- function(x, y, p, num_knots, psd_iwp_vector, pred_step, betaprec = 0.001) {
...
compute_log_likelihood_ospline_seq2 <- function(x, y, p_vec, num_knots, psd_iwp_vector, pred_step, betaprec = 0.001) {
...
fit_ospline_with_prior <- function(x, y, p, num_knots, prior_weight, pred_step, betaprec = 0.001, num_cores = 1) {
  ...
  if (num_cores > 1 && .Platform$OS.type != "windows") {
    ...
    results_list <- mclapply(seq_len(nrow(active_prior)), function(i) {
      ...
      fit_ospline(x, y, p, num_knots, psd_iwp, pred_step, betaprec)
    }, mc.cores = num_cores)
  }
  else {
    ...
    for (i in seq_len(nrow(active_prior))) {
      psd_iwp <- active_prior$psd_iwp[i]
      results_list[[i]] <- fit_ospline(x, y, p, num_knots, psd_iwp, pred_step, betaprec)
    }
  }
...
fit_ospline_with_prior2 <- function(x, y, num_knots, prior_weight, pred_step, betaprec = 0.001, num_cores = 1) {
  # Filter out rows where prior weight is zero
  active_prior <- prior_weight[prior_weight$prior_weight > 0, ]
  ...
  if (num_cores > 1 && .Platform$OS.type != "windows") {
    ...
    results_list <- mclapply(seq_len(nrow(active_prior)), function(i) {
      row <- active_prior[i, ]
      fit_result <- fit_ospline(x, y, row$p, num_knots, row$psd_iwp, pred_step, betaprec)
      fit_result
    }, mc.cores = num_cores)
    # Extract log likelihoods from results
