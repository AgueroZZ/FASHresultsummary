...
compute_log_likelihood_exact <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian){
  ...
  ff <- TMB::MakeADFun(
    ...
    DLL = "Gaussian_theta_known_unequal",
    silent = TRUE
  )
...
fit_exact <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian){
  C <- as.matrix(compute_Wp_cov(x, p = p))
  P = as(solve(C),'dgTMatrix')
  n <- length(x)
  X <- BayesGP:::global_poly_helper(x, p = p)
...
compute_log_likelihood_exact_aug <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian){
  ...
  if(psd_iwp != 0){
    ...
    ff <- TMB::MakeADFun(
      ...
      parameters = tmbparams,
      random = "W",
      DLL = "Gaussian_just_fixed_unequal",
      silent = TRUE
    )
  }
...
fit_exact_aug <- function(x, y, p, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian){
  if(psd_iwp != 0){
    P = Compute_Aug_Wp_Prec(x, p = p)
    X = BayesGP:::global_poly_helper(x, p = p)
    B = Compute_design_Aug(x, p = p)
...
compute_log_likelihood_ospline <- function(x, y, p, num_knots = 100, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian){
...
fit_ospline <- function(x, y, p, num_knots = 100, psd_iwp, pred_step, betaprec = 0.001, sd_gaussian){
  knots <- seq(min(x), max(x), length=num_knots)
  X <- BayesGP:::global_poly_helper(x, p = p)
  P <- BayesGP::compute_weights_precision_helper(knots)
  B <- BayesGP:::local_poly_helper(knots = knots, refined_x = x, p = p)
...
compute_log_likelihood_ospline_seq <- function(x, y, p, num_knots, psd_iwp_vector, pred_step, betaprec = 0.001, sd_gaussian) {
...
compute_log_likelihood_ospline_seq2 <- function(x, y, p_vec, num_knots, psd_iwp_vector, pred_step, betaprec = 0.001, sd_gaussian) {
...
compute_log_likelihood_exact_aug_seq <- function(x, y, p_vec, psd_iwp_vector, pred_step, betaprec = 0.001, sd_gaussian) {
  # Initialize a list to store the results for each p
  all_log_likelihoods <- c()
...
### Fit a sequence of k models based on a prior matrix using ospline: (when psd_iwp is varying)
fit_ospline_with_prior <- function(x, y, p, num_knots, prior_weight, pred_step, betaprec = 0.001, sd_gaussian, num_cores = 1) {
...
fit_exact_aug_with_prior <- function(x, y, p, prior_weight, pred_step, betaprec = 0.001, sd_gaussian, num_cores = 1) {
  # Filter out rows where prior weight is zero
  active_prior <- prior_weight[prior_weight$prior_weight > 0, ]
...
### Fit a sequence of k models based on a prior matrix using ospline: (when both psd_iwp and p are varying)
fit_ospline_with_prior2 <- function(x, y, num_knots, prior_weight, pred_step, betaprec = 0.001, sd_gaussian, num_cores = 1) {
  # Filter out rows where prior weight is zero
  active_prior <- prior_weight[prior_weight$prior_weight > 0, ]
...
fit_exact_aug_with_prior2 <- function(x, y, prior_weight, pred_step, betaprec = 0.001, sd_gaussian, num_cores = 1) {
  # Filter out rows where prior weight is zero
  active_prior <- prior_weight[prior_weight$prior_weight > 0, ]
