...
- There are $N$ sets of series data: $\boldsymbol{y} = \{y_i(t_j): j\in[n_j]\}_{i=1}^{N}$, where $n_j$ is the length of the $j$-th series.
...
- **Cause-specific Mortality Data**: FASH can be used to model different cause-specific mortality series over time (for example, in Canada), and identify the causes with similar mortality pattern.
- **Financial Data**: FASH can be used to smooth and group the stock price trends at different companies.
