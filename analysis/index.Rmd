...
For now, let's assume the mean function $m_k$ is zero, and each GP component defined through the following ordinary differential equation (ODE):
$$Lf(t) = \sigma_k W(t),$$
...
Specifically, this prior shrinks the function $g$ toward the base model $\text{Null}\{L\}$, which is the set of functions that satisfy $Lf = 0$.
...
When all the observations are Gaussian, the posterior mean $\mathbb{E}(f|\boldsymbol{y}_i)$ using the second order IWP is shown to be exactly the cubic smoothing spline estimate in [Kimeldorf and Wahba, 1970](https://www.jstor.org/stable/2239347).
...
For now, assume that we are fixing the $L$ operator to be the $p$th order IWP, and only the standard deviation parameter $\sigma_k$ is varying across different GP components.
Following the practice of [Stehpens, 2017](https://academic.oup.com/biostatistics/article/18/2/275/2557030?login=true) and [Urbut et al, 2018](https://www.nature.com/articles/s41588-018-0268-8), we use a dense grid of $\{\sigma_k:k\in[K]\}$, and use empirical Bayes to estimate the prior mixture weights $\boldsymbol{\pi} = \{\pi_1,...,\pi_K\}$.
...
$$\hat{\boldsymbol{\pi}} = \arg\max_{\boldsymbol{\pi}} \sum_{i=1}^{N} \log\left(\sum_{k=1}^{K} \pi_k \mathbf{L}_{ik}\right).$$
This can be solved using either the interior point method or the Sequential Quadratic Programming (SQP) method ($\texttt{mixqp}$) in [Kim et al, 2020](https://www.tandfonline.com/doi/full/10.1080/10618600.2019.1689985).
...
$$p(f_i(t)|\boldsymbol{y}, \hat{\boldsymbol{\pi}}) = \sum_{k=1}^{K} \tilde{\pi}_k p(f_i(t)|\boldsymbol{y}_i,\sigma_k),$$
where the posterior mixture weights $\tilde{\pi}_k$ is defined as $\tilde{\pi}_k = \frac{\hat{\pi}_k \mathbf{L}_{ik}}{\sum_{k=1}^{K} \hat{\pi}_k \mathbf{L}_{ik}}$.
...
In this way, the complicated integration required in the posterior computation is replaced by simpler optimization tasks with sparse matrices.
...
- **COVID Mortality Data**: FASH can be used to infer and group the COVID mortality rate at different countries (or states), and identify the countries (states) that have similar mortality rate trend.
- **Cause-specific Mortality Data**: FASH can be used to model the mortality series over time for different type of causes (for example, in Canada), and identify the causes that have similar mortality rate trend.
- **Financial Data**: FASH can be used to infer and group the stock price trend at different companies, and identify the companies that have similar stock price behavior.
- **Environmental Data**: FASH can be used to model the concentration of different pollutants overtime, and identify type of pollutants that have similar behavior.


In some applications above, when each series has a same number of observations and the observations are equally spaced, it is possible to directly apply the $\texttt{mash}$ approach in Urbut et al, 2018.
However, when the observations are not equally spaced or when each series has different number of observations, the $\texttt{mash}$ approach is not directly applicable.

### **Example**
*[Simulation](simulation.html)*: We will simulate the data and show the observed time series for each category of pollutants.
