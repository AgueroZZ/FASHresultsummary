...
where $v_{ij}$ is the fixed effect that denotes the weekdays, and the prior on $f_i$ is a finite mixture of IWP-3 with different levels of smoothness (PSD).
...
for (country in cluster1_countries[1:13]) {
...
```{r}
par(mfrow = c(4, 4), mar = c(2, 2, 1, 1))  
for (country in cluster2_countries[1:15]) {
  agg_result <- fitted_datasets[[country]]
  plot(datasets[[country]]$Date, agg_result$median, type = 'l',  
       main = paste0(country), xlab = "", ylab = "", col = "blue",
       cex = 0.5, ylim = c(0,max(datasets[[country]]$y)*1.2),
...
for (country in cluster3_countries[1:5]) {
  agg_result <- fitted_datasets[[country]]
  plot(datasets[[country]]$Date, agg_result$median, type = 'l',  
       main = paste0(country), xlab = "", ylab = "", col = "blue",
       cex = 0.5, ylim = c(0,max(datasets[[country]]$y)*1.2),
