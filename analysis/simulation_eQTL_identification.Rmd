...
To compare their rankings of eQTLs based on the cumulative FDA, we can compare the rank distribution of each type of eQTLs (A,B,C) based on the two methods:

```{r}
fda_df_merge <- rbind(fda_df %>% mutate(method = "FASH"), fda_df_mash %>% mutate(method = "MASH"))

ggplot(fda_df_merge, aes(x = type, y = rank, color = type)) +
  geom_boxplot() +
  labs(x = "Type", y = "Rank") +
  theme_minimal() +
...
Overall, MASH does better job at controlling an actual FDA closer to the nominal level, but FASH does better job at keeping the actual FDA lower than the nominal level.
