...
Now, we will replicate the above analysis for each cell line, and see if we observe similar patterns across different cell lines.


#### **Difference in Gene Expression Trajectories Across Cell Lines**

First, let's take a look at the inferred $f_i^{(l)}(t)$ for each gene (indexed by $i$) in each cell line (indexed by $l$).
...
plot_gene_expression_log_across_cell_lines <- 
  ...
  if (!color) {
    pink_color <- rgb(1, 0.4, 0.6, alpha = 0.2)  # Define a pink color with transparency
    p <- p +
      geom_line(size = 0.5, color = pink_color) +  # Set line color explicitly
      guides(color = "none")
  }else{
    p <- p + geom_line(size = 0.5, aes(color = CellLine))
  }
...
Let's first look at `ANGIOGENESIS` gene set that is enriched in cluster 9 (identified from our previous clustering result based on the largest cell-line) across cell lines. There are 8 genes in the `ANGIOGENESIS` gene set that are also in cluster 9.
...
# modify the figure so they all have the same y_lim:
# range_choice <- range(unlist(lapply(plot_list_ANGIOGENESIS, function(x) range(x$data$Median))))
# log_range_choice <- log10(range_choice)
log_range_choice <- c(-8,-1.5)
for (i in 1:length(plot_list_ANGIOGENESIS)) {
plot_list_ANGIOGENESIS[[i]] <- plot_list_ANGIOGENESIS[[i]] +
    scale_y_log10(limits = 10^log_range_choice)  # Specify limits on the transformed scale
}

do.call(gridExtra::grid.arrange, c(plot_list_ANGIOGENESIS, ncol = 4))
```

Note that for each gene, the inferred trajectory $f_i^{(l)}(t)$ shows variation across cell lines. 
However, they all show similar types of non-linearity especially in the early time points and in the late times with the "tail oscillation".

Let's then compare that with the results of the gene set `BILE_ACID_METABOLISM` that is enriched in cluster 1. There are 6 genes overlapping between the `BILE_ACID_METABOLISM` gene set and cluster 1.
...
# modify the figure so they all have the same y_lim:
# range_choice <- range(unlist(lapply(plot_list_BILE_ACID_METABOLISM, function(x) range(x$data$Median))))
# log_range_choice <- log10(range_choice)
log_range_choice <- c(-8,-1.5)
for (i in 1:length(plot_list_BILE_ACID_METABOLISM)) {
plot_list_BILE_ACID_METABOLISM[[i]] <- plot_list_BILE_ACID_METABOLISM[[i]] +
    scale_y_log10(limits = 10^log_range_choice)  # Specify limits on the transformed scale
}
...
The result is by large consistent with what we observed for the `ANGIOGENESIS` gene set. 
Although the inferred trajectory $f_i^{(l)}(t)$ shows variation across cell lines, but they are all much more "linear" than what we observed in the `ANGIOGENESIS` gene set.

#### **Comparing Smoothness Across Cell Lines**

To more quantitatively compare the smoothness of the inferred trajectories across cell lines, we can compute the expected value of the smoothing parameter ${\sigma}_i^{(l)}$ for the $i$th gene in the $l$th cell line, which will be denoted as ${\hat{\sigma}}_i^{(l)} = \mathbb{E}[\sigma_i^{(l)}|\mathbf{y}^{(l)}]$.

First, let's see how the distribution of ${\hat{\sigma}}_i^{(l)}$ will differ between the `ANGIOGENESIS` gene set in cluster 9 and the `BILE_ACID_METABOLISM` gene set in cluster 1.
...
plot2 <- ggplot(long_data, aes(x = Gene, y = SigmaMean, fill = Gene)) +
  ...
  geom_jitter(width = 0.2, size = 1, alpha = 0.5) +
  labs(
    title = "Bile Acid Metabolism: Cluster 1",
    x = "Gene",
    y = "Sigma Mean",
    fill = "Gene"
  ) +
...
Based on the above figure, even when we consider different cell-lines, the genes in the `ANGIOGENESIS` gene set in cluster 9 tend to have much higher values of ${\hat{\sigma}}_i^{(l)}$ compared to the genes in the `BILE_ACID_METABOLISM` gene set in cluster 1. 
This confirms our previous observation that the inferred trajectories for the `ANGIOGENESIS` gene set tend to be more non-linear compared to the `BILE_ACID_METABOLISM` gene set.


Then, let's check how the distribution of ${\hat{\sigma}}_i^{(l)}$ changes across cell lines for a specific set of genes:
...
Indeed, there appears to be strong cell-line effects on the smoothness of the inferred trajectories.
For example in the `ANGIOGENESIS` gene set in cluster 9, the genes tend to have much higher values of ${\hat{\sigma}}_i^{(l)}$ in cell-line 8, but much lower values in cell-line 12.

One conjecture is that this might be due to some dynamic eQTLs which modify the expression trajectories of the genes in the `ANGIOGENESIS` gene set in cluster 9 across different cell lines.



#### **Comparing Structure Plots Across Cell Lines** {.tabset .tabset-pills}

In our above comparisons, we have been using the clustering results from the largest cell-line (cell-line 1) to construct various plots across cell lines.
Now, let's confirm that the clustering results are mostly consistent across different cell lines by comparing the structure plots across different cell lines.

##### Cell Line Index  1

```{r, echo=FALSE, message=FALSE, warning=FALSE}
cell_line_index <- 1
all_gene <- unique(names(fitted_datasets))
...
Based on the above results, the smoothness of $f_i^{(l)}(t)$ is mostly consistent across all cell lines ($\forall l\in[19]$). 
The gene-set showing larger non-linearity in the first cell-line such as Angiogenesis, still tends to show similar level of non-linearity in other cell lines.
However, the exact trajectories of the gene-set still vary across cell lines, which could indicate the existence of certain dynamic eQTLs effects.
