...
To model the true (log) expression level $f_i(t_j)$ of the $i$th gene at day $t_j$, we assume that the expression count $y_{i}(t_j)$ is Poisson distributed with mean $\exp \left(f_i(t_j)\right)$, where the prior on $f_i$ is a finite mixture of IWP-2 with different levels of smoothness (PSD).
...
### **Question: what genes have linear trajectories? **
