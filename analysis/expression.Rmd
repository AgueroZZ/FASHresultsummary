...
plot_gene_expression_log <- function(gene) {
  ...
  plot(datasets[[gene]]$Day, agg_result$median, type = 'l', 
       main = paste0(gene), xlab = "Day", ylab = "Expression", col = "blue",
       cex = 0.5, 
       log = "y", ylim = c(log_min_y, log_max_y),
       cex.main = 0.8, cex.lab = 0.7, cex.axis = 0.7)
...
plot_min <- fitted_datasets %>% lapply(function(x) min(x$median/exp(log_size_vec))) %>% unlist() %>% min()
plot_max <- fitted_datasets %>% lapply(function(x) max(x$median/exp(log_size_vec))) %>% unlist() %>% max()
