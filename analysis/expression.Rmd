...
quantiles <- c(0, 0.1, 0.2, 0.35, 0.5, 1, 3)
...
enrich_cluster <- function(cluster_id, q_val_cutoff = 0.05, pvalueCutoff = 0.05, melted_data) {
  genes_selected <- melted_data %>% filter(cluster == cluster_id) %>% pull(id) %>% as.character() %>% unique()
  ...
  enrich_res <- enricher(entrez_gene_list, TERM2GENE = m_t2g, qvalueCutoff = q_val_cutoff, pvalueCutoff = pvalueCutoff)
  enrich_res
}
```
...
for (i in 1:num_clusters) {
  ...
  enrich_res <- enrich_cluster(cluster_id = i, q_val_cutoff = 0.05, melted_data = melted_data_quantiles)  # Adjust parameters as needed
...
for (cluster in names(enriched_list)) {
  for (term in all_enriched_terms_clean) {
    ...
    original_term <- paste0("HALLMARK_", term)  # Add back the prefix for comparison
    term_in_cluster <- enriched_list[[cluster]]$Description == original_term
    
    # If the term is enriched, fill in the p-value; otherwise, compute the enrichment and add its p-value
    if (any(term_in_cluster)) {
      enrichment_table[term, cluster] <- round(enriched_list[[cluster]]$PValue[term_in_cluster], 4)
    } else {
  ...
  entrez_gene_list <- genes_converted$entrezgene_id
      ...
      enrich_res_full <- enricher(entrez_gene_list, TERM2GENE = m_t2g, pvalueCutoff = 2)
      specific_term <- enrich_res_full %>% filter(Description == original_term)
      
      # Add p-value if it exists
      if (nrow(specific_term) > 0) {
        enrichment_table[term, cluster] <- round(specific_term$p.adjust, 4)
      } else {
